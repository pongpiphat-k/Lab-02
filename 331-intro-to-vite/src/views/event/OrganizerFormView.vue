...
export default {
  ...
  methods: {
    async saveOrganizer() {
      try {
        const response = await axios.post("http://localhost:8080/organizer", this.organizer);
        this.message = 'You successfully added a new organizer: ' + response.data.organizer;
        
      } catch (error) {
        this.message = "Error saving organizer.";
        console.error(error);
      }
    }
  }
...
<template>
  <div>
    ...
    <form @submit.prevent="saveOrganizer">
      ...
      <input v-model="organizer.location" type="text" placeholder="Name" class="field" />
